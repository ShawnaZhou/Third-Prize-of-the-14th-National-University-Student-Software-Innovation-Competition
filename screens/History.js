...
const History = () => {
  ...
  const [date, setDate] = useState(new Date(1451577600000));
  const [show, setShow] = useState(false);
  const [Value, setDateValue] = useState(new Date(1451577600000));
  const [firstTime, setFirstTime] = useState(
    new Date(1451577600000).toString(),
  );
  const [lastTime, setLastTime] = useState(new Date(1452096000000).toString());
  ...
  const waterDataFormatter = e => {
    ...
    console.log('formateWaterJson: ', data);
    setWaterData(data);
  };
  const gasDataFormatter = e => {
    ...
    let data = [timeData, valueData];
    console.log('formateElectricJson: ', data);
    setGasData(data);
  };
  const powerDataFormatter = e => {
    ...
    let data = [timeData, valueData];
    console.log('formateGasJson: ', data);
    setPowerData(data);
  };
    ...
    <SafeAreaView style={{height: '100%', width: '100%'}}>
      <Card>
        <Card.Content style={{flexDirection: 'row'}}>
          <View style={{width: '45%'}}>
            <Title>History</Title>
            <Paragraph>Observe the data transition in history...</Paragraph>
            <Button
      ...
      <ScrollView style={{flex: 1}}>
        <Card style={{marginTop: 10, height: 600}}>
          <Card.Content>
            <HistoryCharts pushData={pushData} />
          </Card.Content>
        </Card>
      </ScrollView>
      {show && (
